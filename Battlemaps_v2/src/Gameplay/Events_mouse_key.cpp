...
bool MOUSE_HOVERING(sf::Event& event, Event_states& event_state) {
	return (event_state == neutral && event.type == sf::Event::MouseMoved);
}
...
bool SELECTING_PIECE(sf::Event& event, Event_states& event_state) {
	return (event_state == neutral && event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left);
}
...
bool RELEASING_PIECE(sf::Event& event, Event_states& event_state) {
	return ((event_state == moving_piece || event_state == selected_piece) && (event.type == sf::Event::MouseButtonReleased)) ;
}